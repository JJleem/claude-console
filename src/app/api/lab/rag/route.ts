  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        // 2. Score with TF-IDF cosine similarity
        ...
        // Sort by score desc, keep top 3 as context
        const sorted = [...scored].sort((a, b) => b.score - a.score);
        const top3 = sorted.slice(0, 3);

        send({ type: "chunks", data: scored });

        // 3. Stream Claude response with injected context
        ...
        const msgStream = anthropic.messages.stream({
          ...
          max_tokens: 1024,
          system:
            "당신은 제공된 컨텍스트를 기반으로 질문에 답하는 도우미입니다. 반드시 컨텍스트 내용만 사용하여 답변하고, 컨텍스트에 없는 내용은 추측하지 마세요.",
          messages: [
            {
              role: "user",
              content: `다음 컨텍스트를 참고하여 질문에 답해주세요.\n\n컨텍스트:\n${contextText}\n\n질문: ${query}`,
            },
          ],
        });
